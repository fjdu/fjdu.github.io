...
假设 \(m\) 是最后分类器弄错的样本个数。这 \(m\) 个样本里的每一个至少被弄错 \(T/2\) 次，所以每个的权重至少为 \((1+\varepsilon)^{T/2}\)。也就是说总的权重至少为 \(m(1+\varepsilon)^{T/2}\)。另一方面，在时刻 \(t+1\)，只有那些在 \(t\) 时刻被错误分类的样本的权重被提高了。按照弱学习的性质，\(t\) 时刻被错误分类的样本的总权重至多为总权重的 \(f=(\frac{1}{2}-\gamma)\)。令 \(\text{weight}(t)\) 表示 \(t\) 时刻的总权重，则
\begin{equation}
\begin{split}
