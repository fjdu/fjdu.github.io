...
<p>
在上面的描述中，\(A\)是可完整获得的，我们希望通过\(U\)和\(V\)来找出基础矢量。但在现实情形，比如在电影推荐的情形，每个顾客可能只看了少量电影，也就是说更自然的假设是我们只知道\(A\)的一部分元素，然后希望估计完整的\(A\)。如果\(A\)是个一般的\(n\times d\)矩阵，则需要的存储空间是\(\Omega(n d)\)的量级。如果假定\(A\)是一个低秩矩阵叠加上了一部分噪音，并且其元素是从某个分布随机取样的，则有可能可以用SVD来估计\(A\)的总体。这被称为协同过滤，这被用于推荐系统。
</p>
