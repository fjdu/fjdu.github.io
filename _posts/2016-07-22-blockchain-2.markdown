...
- 另一个例子是二步承诺协议 (two-phase commit; 2PC)，经常在数据库环境中出现。第一步被称为交易准备，而在第二步，交易要么被承诺，要么被中止。这个 2PC 过程不是由客户发起，而是由一个指定的被称为协调器的服务器节点发起。
...
- 我们能对凭证做什么？我们能否简单地把算法 2.10 里的锁替换成凭证？我们需要添加至少一个额外的步骤，因为只有客户知道在第二步中是否多数凭证是有效的。
...
<ul>
<li>这个算法有些问题：假设客户 \(u_1\) 第一个将其指令 \(c_1\) 保存在多数服务器上。假定 \(u_1\) 刚好在通知服务器之前 (第三步开始之前) 变得很慢，而另一个客户 \(u_2\) 将某些服务器上存储的指令更新为 \(c_2\)。之后，\(u_1\) 告诉让服务器执行存储的指令。现在，有些服务器执行 \(c_1\)，有些执行 \(c_2\)！</li>
<li>怎么解决这个问题？我们知道每个在 \(u_1\) 之后更新存储指令的客户 \(u_2\) 必定比 \(u_1\) 使用了更新的凭证。既然在第二步 \(u_1\) 的凭证被接收，\(u_2\) 必定是在 \(u_1\) 往相应的服务器上存储指令之后才获取它的凭证的。</li>
<li>想法：如果在第一步中服务器不是只分发凭证，而且把当前存储的指令告知客户，会怎么样？这样，\(u_2\) 会知道 \(u_1\) 已经存储了 \(c_1\)，因此 \(u_2\) 可以支持 \(u_1\)，也存储 \(c_1\)，而不是 \(c_2\)。既然两个客户试图存储和执行相同的指令，顺序就不成问题了。</li>
<li>可如果不是所有的服务器都保存了相同的指令，并且 \(u_2\) 在第一步发现了多个存储的指令，\(u_2\) 应该支持哪个？</li>
<li>注意到，支持最新存储的指令总是安全的。只要不存在多数派，客户就可以支持任何指令。但是，当出现多数派之后，客户需要支持这个值。</li>
<li>所以，为了决定哪个指令是最新存储的，服务器可以记下用来存储每个指令的凭证号码，之后在第一步把这个号码告诉客户。</li>
<li>如果每个服务器使用其自身的凭证号，那么最新的凭证未必拥有最大的数字。这个问题可通过客户自己对凭证号给出建议来解决。</li>
</ul>

<p>
<hr>
<b>算法 2.13</b> Paxos<br>
<pre><code>
<table>
<tr>
    <th>客户 (提议者)</th>
